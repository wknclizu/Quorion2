create or replace TEMP view c1 as select src, count(*) as cnt from google group by src; 
create or replace TEMP view c2 as select src, count(*) as cnt from google group by src;
create or replace TEMP view f1 as select g1.*, cnt as c1cnt from google g1 join c1 using(src);
create or replace TEMP view f2 as select g3.*, cnt as c2cnt from google g3 join c2 on g3.dst=c2.src;
create or replace TEMP view f3 as select dst, min(c1cnt) as mf1 from f1 group by dst;
create or replace TEMP view f4 as select mf1, g2.* from google g2 join f3 on g2.src=f3.dst;
