create or replace TEMP view g3 as select google.src as v4, google.dst as v6, v10 from google, (SELECT src, COUNT(*) AS v10 FROM google GROUP BY src) AS c2 where google.dst = c2.src;
create or replace TEMP view g3Aux8 as select v4, v6, v10 from g3;
create or replace TEMP view g1 as select google.src as v7, google.dst as v2, v8 from google, (SELECT src, COUNT(*) AS v8 FROM google GROUP BY src) AS c1 where google.src = c1.src;
create or replace TEMP view minView7866219079596252654 as select v2, min(v8) as mfL2980243762834534920 from g1 group by v2;
create or replace TEMP view joinView3477189909482605523 as select src as v2, dst as v4, mfL2980243762834534920 from google AS g2, minView7866219079596252654 where g2.src=minView7866219079596252654.v2;
