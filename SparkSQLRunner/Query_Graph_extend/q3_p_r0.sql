...
create or replace TEMP view aggView4743162493683758847 as select v2, COUNT(*) as annot from g1 group by v2;
create or replace TEMP view aggJoin4542444878411930196 as select v4, annot from g2 join aggView4743162493683758847 using(v2);
...
create or replace TEMP view aggView5459960029595792617 as select v4, SUM(annot) as annot from aggJoin4542444878411930196 group by v4;
create or replace TEMP view aggJoin3593273221663519992 as select annot from g3 join aggView5459960029595792617 using(v4);
