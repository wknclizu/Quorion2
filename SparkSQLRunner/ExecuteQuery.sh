...
function execute_sparksql {
    ...
    do
        timeout -s SIGKILL 2h ${spark_submit} --class SparkSQLRunner --master "local[${parallelism}]" --driver-memory 4G --executor-memory 4G --conf "spark.shuffle.service.removeShuffle=true" --conf "spark.local.dir=/home/data/spark_temp" ${sparkJar} ${dataPath} "${queryPath}" "${SCRIPT_PATH}/Schema/${schema}.sql" >> ${log_file} 2>&1
